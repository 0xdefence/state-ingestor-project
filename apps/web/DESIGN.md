...
Current dependency availability and canonical effect are server projections, separate from the immutable classification readiness and the latest manual decision. Automatically promoted former blockers leave awaiting-review counts and filters. Conflict evidence resolves canonical revisions back to their candidate fields and shows current/prior values against proposed values under the matching business identifier; unrelated dependency records are labelled separately. Expected normalisation domains arrive with issue evidence from the registered server normalisers. Run-bound details reject cross-file selections, and malformed scope/filter URLs recover with an explanation.
